...
function mostrar(){
  ...
  for (let index = inicioPag; index < inicioPag + 20 && index<data.length; index++) {
    ...
    if (element.imagenesAdicionales.length > 0){
        ...
        anchor.innerHTML='ver más imágenes';
        anchor.href=`/imagenes-adicionales/${element.id}`
        // boton.classList.add("abrir-modal");
        divObra.append(anchor)
